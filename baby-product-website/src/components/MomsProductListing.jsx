...
import products from '../data/momsData';
import { FaAngleDown, FaAngleUp } from 'react-icons/fa';

const MomsProductListing = ({ filterState }) => {
    ...
    const sortedProducts = useMemo(() => {
        ...
        let filteredList = products.filter(product => {
            ...
            // gender filter (updated logic for moms & baby care)
            if (gender) {
                if (gender === 'Boy' && !productName.includes('boy')) {
                    return false;
                }
                if (gender === 'Girl') {
                    if (productName.includes('boy') || productName.includes('male')) {
                        return false;
                    }
                }
                if (product.age === 'Maternity' && gender === 'Boy') {
                    return false;
                }
            }
            ...
            // age filter (maternity handling)
            if (ageGroup) {
                if (ageGroup === 'Maternity' && product.age !== 'Maternity') {
                    return false;
                } else if (ageGroup !== 'Maternity' && !product.age.includes(ageGroup.split(' ')[0].replace('-', ''))) {
                    return false;
                }
            }
            ...
            // collection filter (updated logic)
            if (collection) {
                if (collection === 'Trending now' && !product.isTrending) return false;
                if (collection === 'Fast moving' && !product.isFastMoving) return false;
                if (collection === 'Extra warm' && !productName.includes('warm') && !productName.includes('quilted')) return false;
            }
    ...
    }, [sortBy, filterState]); // recalculate when sortBy or filterState changes
        ...
        <div className="product-listing-area">
            ...
            <div className="row align-items-center mb-4 product-listing-header">
                ...
                <div className="col-auto">
                    <h1 className="product-listing-heading m-0">Moms & Baby Care</h1>
                </div>
...
export default MomsProductListing;
